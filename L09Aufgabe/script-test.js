...
(function (L08Coronatime) {
    ...
    function drawTissue() {
        console.log("Tissue");
        var gradient = c.createLinearGradient(0, 0, 0, c.canvas.height);
    ...
    function drawHumanCells(_position, _size) {
        ...
        for (var i = 0; i < 5; i++) { //max 7 antibodies in this area
            var x = Math.random() * window.innerWidth;
            var y = 100 + (10 * Math.random()); //limits range of antibodies in area
            createHumanoid(x, y);
            ...
            function createHumanoid(_x, _y) {
                var r1 = 40;
                var r2 = 70;
                var gradient = c.createRadialGradient(0, 0, r1, 0, 0, r2);
    ...
    function drawCoronavayrus(cx, cy, spikes, outerRadius, innerRadius) {
        var rot = Math.PI / 2 * 3;
        var x = cx * window.innerWidth;
        var y = cy * window.innerHeight;
    ...
    function drawAntibodies(_position, _size) {
        ...
        for (var i = 0; i < 7; i++) { //max 7 antibodies in this area
            var x = Math.random() * window.innerWidth / 1.7; // divides range of spread by 1.7
            ...
            // HUMAN CELLS + VECTOR & SIZE
            function drawTKiller(_position, _size) {
                console.log("TKiller", _position, _size);
                for (var i_1 = 0; i_1 < 1; i_1++) { //max 7 antibodies in this area
                    var x_1 = Math.random() * window.innerWidth;
                    var y_1 = 600 + (100 * Math.random()); //limits range of antibodies in area
                    createTKills(x_1, y_1);
                    if (i_1 < 5) { // max 5 antibodies in this area
                        x_1 = Math.random() * window.innerWidth * 2;
                        y_1 = 500 + (10 * Math.random()); //limits range of antibodies in area
                        createTKills(x_1, y_1);
                    }
                    function createTKills(_x, _y) {
                        var r1 = 20;
                        var r2 = 35;
                        var gradient = c.createRadialGradient(0, 0, r1, 0, 0, r2);
